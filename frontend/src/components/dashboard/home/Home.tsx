...
import { PLASTIC_DEPOSIT_TX } from "../../../utils/queries";
...
const Home = (props: Props) => {
  ...
  const {
    ...
    loading,
    error,
  } = useQuery(PLASTIC_DEPOSIT_TX, {
    variables: { address },
  });

  console.log(depositTxns, error, address);
  ...
  return (
    <section className="w-full px-4">
      <div className="flex flex-row justify-between items-center w-full gap-x-4 px-2 py-4">
        <div className="">
          <Disbursement />
        </div>
        <div className="flex-auto text-right">
          Total Recycled: {depositTxns?.allPlasticDeposited?.length || 0}
        </div>
      </div>
